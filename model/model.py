    ...
    def getArtistaMaggioreInfluenza(self):
        ...
        for artist in self.grafo.nodes():
            self.mappaArtista_influenza[artist] = (
                    self.grafo.out_degree(artist, weight="weight")
                    - self.grafo.in_degree(artist, weight="weight"))
        piuPopolare =sorted(self.mappaArtista_influenza, key= self.mappaArtista_influenza.get, reverse = True)[0]
        return piuPopolare
